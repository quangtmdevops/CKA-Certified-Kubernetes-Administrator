...
- ```bash
   ...
   k config set-context --current --namespace exercise-01
   ```
...
- ```bash
   k run web --image=nginx:1.27 --labels="app=web,tier=frontend" --dry-run=client -o yaml > pod.yml
   ```
- add resource config to pod.yml
- ```yaml
   resources:
      requests:
        memory: "64Mi"
...
- `k label po web version=v1`
- recheck: `k get po --show-labels`

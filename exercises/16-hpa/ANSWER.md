...
- `k config set-context --current --namespace=exercise-16`
...
3. Create a Deployment named `load-app` with:
   ...
   - CPU request: 50m
   - CPU limit: 100m
- gen yaml file: `k create deploy load-app --image=nginx:1.27 --port=80 $do > load-app-deployment.yml` and add this: 
- ```yaml
  resources:
    requests:
      cpu: "50m"
...
- `k expose deployment load-app --port=80 --target-port=80 --name=load-svc`
5. Create an HPA targeting `load-app`:
   - Min replicas: 1
   - Max replicas: 5
   - Target CPU utilization: 50%
- `k autoscale deploy load-app --cpu=50% --min=1 --max=5`

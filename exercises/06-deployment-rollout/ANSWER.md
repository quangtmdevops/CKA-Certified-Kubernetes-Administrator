...
- `k config set-context --current --namespace exercise-06`
- recheck `k config get-contexts`
  2. Create a Deployment named `webapp` with:
     - 3 replicas
     - Image: `nginx:1.28`
     - Strategy: RollingUpdate with maxSurge=1, maxUnavailable=0
     - Record the change cause
  - create deployment manifest `k create deploy webapp --image=nginx:1.28 --replicas=3 --dry-run=client -o yaml > webapp-deployment.yml`
    - add strategy config to `webapp-deployment.yml`
  - ```yaml
    strategy:
    type: RollingUpdate
    rollingUpdate:
...
- recheck number of replicas `k get po`

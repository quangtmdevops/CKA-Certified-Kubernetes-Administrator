...
- `k create ns exercise-03 && kn exercise-03`
2. Create a ConfigMap named `app-config` with:
   - Key `APP_MODE` = `production`
   - Key `LOG_LEVEL` = `debug`
- `k create configmap app-config --from-literal=APP_MODE=production --from-literal=LOG_LEVEL=debug` or `k create cm app-config --from-literal=APP_MODE=production --from-literal=LOG_LEVEL=debug`
...
- create manifest: `k run app --image=busybox:1.36 --namespace=exercise-03 $do > app-pod.yaml`
...
- `k exec app -- env | grep -E "APP_MODE|LOG_LEVEL|DB_USER|DB_PASS"`
6. Verify the mounted files exist at `/etc/config/`
- `k exec app -- ls -l /etc/config`
## Hints
